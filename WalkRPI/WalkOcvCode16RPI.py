...
&Logic輸出來加了給RPI的東西
&取消了直接bar的功能，直接給值
#arduino.write(bytes(SpeedPercentFlag,'utf-8'))好像有錯誤(開通連接上arduino時)
&預設數值已改成木棍
&把垂直線過濾掉了
&成功加了轉彎邏輯，如果是90度大右轉是1000，大左轉是-1000
...
#cap=cv2.imread('./GFencePhoto/GrassFence9.jpg')
'''
#照片檔案跟定義
...
def FindcolorBar(imgNow): #TrakerBar for找到正確的刪顏色參數值，傳入一張RGB照片回傳回傳一張只剩下我們調過濾後的顏色的RGB照片跟遮罩imgMask
    ...
    cv2.setTrackbarPos('HueMax', 'TrackBar1', 42)
    cv2.setTrackbarPos('Saturation Min', 'TrackBar1', 38)
    ...
    cv2.setTrackbarPos('CannyTopThr', 'TrackBar1', 142)
...
def FindHoughLine(CannyImgNow):#傳入一個輪廓img(CannyImg)，傳出那幾個有線的方程式，回傳average line(left_line,right_line)並且畫出所有line跟averageline
    ...
    if lines is not None:
        ...
        '''
        if TurningState is not None:    
            if TurningState ==-1:
                InSecX,InSecY,copyImgF1=lineIntersection(right_line,parallel_line,copyImgF1)
            elif TurningState == 1:
                InSecX,InSecY,copyImgF1=lineIntersection(left_line,parallel_line,copyImgF1)
        else:
        '''
    ...
    return left_line,right_line,copyImgF1,InSecX,InSecY,parallel_line#此為average line(兩線有各兩點x1,y1/x2,y2)
...
def average_slope_intercept(lines,image):
    ...
    for line in lines:
        for x1, y1, x2, y2 in line:
            ...
            if -0.16<slope<0.16:#如果斜率為+-0.16(即角度是+-9.09)判斷為水平線
                parallel_lines.append((slope, intercept))
                parallel_weights.append((length))
            elif slope>2.5 or slope<-2.5: #角度在68度以上或是-68以下就算在垂直線，過濾掉它
    ...
    global TurningState
    if len(left_weights) == 0 or len(right_weights) == 0:#轉彎時，使否在轉彎時段的判定
        #TurningState=100 #如果突然只剩一條線，不知可以拿來幹嘛
        if len(left_weights) == 0 and len(parallel_weights) > 0 and len(right_weights) > 0:
             TurningState = -1 #就要左大轉彎
        elif len(left_weights) > 0 and len(parallel_weights) > 0 and len(right_weights) == 0:
             TurningState = 1  #就要右大轉彎
    
    if len(left_weights) > 0 and len(right_weights) > 0:#大轉彎一直到左右兩條線都被看到了，才變回一般移動法
        TurningState = 0    
...
def lane_lines(image, lines):
    ...
    '''
    if left_lane is None or right_lane is None:#轉彎時，使否在轉彎時段的判定
        #TurningState=100 #如果突然只剩一條線，不知可以拿來幹嘛
        if left_lane is None and parallel_lane is not None and right_lane is not None:
            TurningState=-1 #就要左大轉彎
        elif left_lane is not None and parallel_lane is not None and right_lane is None:
            TurningState=1  #就要右大轉彎
    
    if left_lane is not None and right_lane is not None:#大轉彎一直到左右兩條線都被看到了，才變回一般移動法
        TurningState = 0
    '''
...
def LogicDetermed(inSecX,inSecY,imgF1):
    ...
    elif TurningState==-1:#要大左轉
            SpeedPercentFlag=-1000#為一0~100的數字，用來給arduino要轉多快
    elif TurningState==1:#要大右轉
            SpeedPercentFlag=1000
...
WetherBarFlag=1#(要做Settling就設為1)
...
while True:
    ...
    if ret:
        ...
        left_line, right_line,copyimgF1,NowInSecX,NowInSecY,parallel_line=FindHoughLine(ImgCanny1)
        ...
        if SumInSecCount<10:#每100次frame取一次平均去修正
            ...
            SumInSecCount=0
        '''
        else: #進入轉彎Mode時，NowInSecX會是水平線跟鞋線的交點
            AverageInSecX=NowInSecX
            AverageInSecY=NowInSecY
        '''
        ...
        SpeedPercentFlag=str(LogicDetermed(AverageInSecX,AverageInSecY,imgF1))#如果是90度大右轉是1000，大左轉是-1000
