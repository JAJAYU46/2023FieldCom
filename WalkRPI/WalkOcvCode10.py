...
#Computercap=cv2.VideoCapture(0) #直接讀取鏡頭，在此寫上"鏡頭編號"，內建電腦鏡頭編號是0，如果外接了一個鏡頭，那外接鏡頭的編號是1
...
def FindCanny(FiltedImgNow):#傳入一張已經過濾過顏色的RGB照片(或是Mask)，回傳照片的邊緣
    #<先找邊緣>
    imgCanny = cv2.Canny(FiltedImgNow, 50, 200)  #先找邊緣canny
...
def FindHoughLine(CannyImgNow):#傳入一個輪廓img(CannyImg)，傳出那幾個有線的方程式，回傳average line(left_line,right_line)並且畫出所有line跟averageline
    ...
    return left_line,right_line,copyImgF1#此為average line(兩線有各兩點x1,y1/x2,y2)
...
#############正式的程式main############
#imgF1為影像處理後的Frame
ColorBarFlage=0#讓FindcolorBar只有第一次被做而已
...
while True:
    ...
    imgF1 = frame.copy()
    if ColorBarFlage==0:
        h_min,h_max,s_min,s_max,v_min,v_max=FindcolorBar(frame)
        ColorBarFlage=1
